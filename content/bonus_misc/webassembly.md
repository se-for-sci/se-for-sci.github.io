...
an opt-in experiment for chrome that loads a directory with user permissions.
JupyterLite keeps its directory in your browser's cache, so it is persistent.
...
separately into WASM and provided by URL - binary compatibility is not stable
enough to be proposed and added to PyPI.
